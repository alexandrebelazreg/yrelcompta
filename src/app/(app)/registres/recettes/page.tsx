...
export default async function RevenueRegisterPage({ searchParams }: { searchParams: Promise<{ annee?: string | string[] }> }) {
  ...
  return <>
    ...
    <header className="page-header"><div><p className="eyebrow">Registre chronologique</p><h1>Livre des recettes</h1><p>Une ligne par encaissement réel, pour l’année {year}.</p></div><div className="header-actions"><form className="year-filter" method="get"><label htmlFor="revenue-year">Année</label><input className="input" id="revenue-year" name="annee" type="number" min="1900" max="9999" defaultValue={year}/><button className="button" type="submit">Afficher</button></form><Link className="secondary-link" href={`/registres/recettes/export?annee=${year}`}>Exporter le livre des recettes CSV</Link></div></header>
    ...
    <p className="dashboard-note">Le montant réglementaire est l’encaissement brut. Les commissions sont informatives et ne réduisent jamais la recette.</p>
    ...
    <section className="register-secondary"><div className="section-heading"><div><p className="eyebrow">Réconciliation séparée</p><h2>Remboursements clients</h2></div></div>
      <p>Ces remboursements sont présentés séparément et ne sont pas déduits automatiquement du livre des recettes ou du montant déclaratif.</p>
