...
export default async function PurchaseRegisterPage({ searchParams }: { searchParams: Promise<{ annee?: string | string[] }> }) {
  ...
  return <>
    ...
    <header className="page-header"><div className="info-line"><h1>Registre des achats</h1><InfoTip label="À propos du registre des achats"><p>Une ligne par règlement fournisseur réel, pour l’année {year}.</p><p>Le montant réglé est affiché en brut. La part professionnelle est une donnée interne YrelCompta et ne constitue pas une déduction fiscale.</p></InfoTip></div><div className="header-actions"><form className="year-filter" method="get"><label htmlFor="purchase-year">Année</label><input className="input" id="purchase-year" name="annee" type="number" min="1900" max="9999" defaultValue={year}/><button className="button" type="submit">Afficher</button></form><Link className="secondary-link" href={`/registres/achats/export?annee=${year}`}>Exporter le registre des achats CSV</Link></div></header>
    ...
    <section className="register-secondary"><div className="section-heading"><div className="info-line"><h2>Avoirs et remboursements fournisseurs</h2><InfoTip label="À propos du traitement des avoirs et remboursements fournisseurs">Ces opérations restent séparées du montant brut du registre des achats et ne sont jamais soustraites silencieusement.</InfoTip></div></div>
