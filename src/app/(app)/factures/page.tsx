...
export default async function InvoicesPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <>
    <header className="page-header"><div><p className="eyebrow">Documents clients</p><h1>Factures et avoirs</h1><p>Documents immuables générés depuis les ventes validées.</p></div></header>
    <p className="dashboard-note">Le PDF généré par YrelCompta n’est pas, à lui seul, une facture électronique au sens de la réforme française. La transmission B2B réglementaire devra passer par une plateforme agréée selon le calendrier applicable.</p>
