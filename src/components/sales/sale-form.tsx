...
import type { Sale, SaleProductOption } from "@/types/sales";
import { Field, TextareaField } from "@/components/ui/field";
import { FormMessage } from "@/components/ui/form-message";
import { SubmitButton } from "@/components/ui/submit-button";

interface EditableItem { key: string; description: string; quantity: string; unitPrice: string; productId: string | null; }
const moneyInput = (cents: number) => (cents / 100).toFixed(2).replace(".", ",");

export function SaleForm({ sale, products }: { sale?: Sale; products: SaleProductOption[] }) {
  const initialItems = sale?.sale_items.map((item) => ({ key: item.id ?? crypto.randomUUID(), description: item.description, quantity: String(item.quantity), unitPrice: moneyInput(item.unit_price_cents), productId: item.product_id }))
    ?? [{ key: crypto.randomUUID(), description: "", quantity: "1", unitPrice: "", productId: null }];
  ...
  function selectProduct(key: string, productId: string) {
    ...
    updateItem(key, product ? { productId: product.id, description: product.name, unitPrice: moneyInput(product.sale_price_cents) } : { productId: null });
  }
  function validate(event: FormEvent<HTMLFormElement>) {
    const form = new FormData(event.currentTarget);
    const result = saleFormSchema.safeParse({ saleId: sale?.id, orderedOn: form.get("orderedOn"), channel: form.get("channel"), customerName: form.get("customerName"), notes: form.get("notes"), shipping, discount, items: items.map(({ description, quantity, unitPrice, productId }) => ({ description, quantity, unitPrice, productId })) });
  ...
  const serializedItems = JSON.stringify(items.map(({ description, quantity, unitPrice, productId }) => ({ description, quantity, unitPrice, productId })));
  return <form action={action} onSubmit={validate} className="sale-form form-stack" noValidate>
    ...
    <fieldset className="sale-lines"><legend>Lignes de vente</legend><p className="field-help">Le coût de fabrication courant sera figé lors de la validation définitive.</p>{items.map((item, index) => <div className="sale-line" key={item.key}>
      <div className="field"><label htmlFor={`item-product-${item.key}`}>Produit du catalogue</label><select className="input" id={`item-product-${item.key}`} value={item.productId ?? ""} onChange={(event) => selectProduct(item.key, event.target.value)}><option value="">Saisie libre</option>{products.map((product) => <option key={product.id} value={product.id}>{product.name}{product.sku ? ` · ${product.sku}` : ""}{product.is_active ? "" : " · Archivé"}</option>)}</select></div>
    ...
    </div>)}<button className="secondary-link add-line" type="button" onClick={() => setItems((current) => [...current, { key: crypto.randomUUID(), description: "", quantity: "1", unitPrice: "", productId: null }])}>+ Ajouter une ligne</button></fieldset>
